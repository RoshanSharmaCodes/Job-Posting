...
export default function Card({ data, apply = false, del = false }) {
  ...
  const onSubmit = async (pitch) => {
    ...
    try {
      fetch(`https://job-posting-server-puce.vercel.app/Jobs/apply-job/${jobId}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(applicantInfo) })
        .then((data) => data.json())
        .then((result) => {
